...
def process_directory(input_dir, character_name, **effect_params):
    ...
    for i, input_file in enumerate(audio_files, 1):
        # Create output filename (keep original name, change to .wav)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Apply radio/walkie-talkie effect to audio files or directories",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  Single file:
    python radio_effect.py input.wav output.wav
    python radio_effect.py input.wav output.wav --highpass 200 --lowpass 3000
    python radio_effect.py input.ogg output.wav --mid-boost 900 --mid-db 8

  Directory processing:
    python radio_effect.py /path/to/audio/dir CharacterName
    python radio_effect.py /home/user/Downloads/VO_ZhaoHui_JP ZhaoHui
    python radio_effect.py ./audio_files MyCharacter --highpass 200
        """,
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=200,
        help="High-pass filter frequency in Hz (default: 200, try 100-300)",
    )
    parser.add_argument(
        "--lowpass",
        type=float,
        default=4000,
        help="Low-pass filter frequency in Hz (default: 4000, try 3000-5000)",
    )
    parser.add_argument(
        "--mid-boost",
        type=float,
        default=800,
        help="Midrange boost center frequency in Hz (default: 800, try 600-1200)",
    )
    parser.add_argument(
        "--mid-db",
        type=float,
        default=4,
        help="Midrange boost amount in dB (default: 4, try 2-10)",
    )
    parser.add_argument(
        "--boost-q",
        type=float,
        default=1.0,
        help="Q factor for midrange boost (default: 1.0, higher = narrower)",
    )
